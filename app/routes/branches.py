...
@router.get("/branches/{branch_id}/sim-cards", response_model=dict, summary="Получить количество доступных симкарт в конкретном филиале", description="Возвращает количество доступных симкарт в конкретном филиале. В хедере запроса необходимо использовать Bearer token, полученный при выполнении операции логина, а в cookie необходимо передавать x_id_session, полученный в предыдущем запросе /branches")
